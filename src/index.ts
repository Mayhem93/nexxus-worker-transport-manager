...
import { NexxusTransportManagerWorker, type NexxusTransportManagerWorkerConfig } from '@mayhem93/nexxus-worker-lib';
...
(async () => {
  ...
  await configManager.validateServices([NexxusTransportManagerWorker, NexxusRedis]);

  const workerConfig = configManager.getConfig('app') as NexxusTransportManagerWorkerConfig;

  const LoggerClass = await NexxusTransportManagerWorker.resolveFactoryService(configManager, workerConfig.logger);
  const DbClass     = await NexxusTransportManagerWorker.resolveConstructableService(configManager, workerConfig.database);
  const MqClass     = await NexxusTransportManagerWorker.resolveConstructableService(configManager, workerConfig.message_queue);
  ...
  const worker   = new NexxusTransportManagerWorker({ configManager, logger, database: db, messageQueue: mq, redis });
  ...
  if (logger) {
    logger.emerg(message, 'NxxTransportManager');
  } else {
